...
knitr::opts_chunk$set(eval = !is_check,fig.align="center",collapse=TRUE,results="hold")
```
...
meta = "https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE85210&targ=gsm&form=text&view=brief"
meta = readLines(meta)
...
meta = meta[,.(gsm,smoker=factor(`subject status`),red,grn)]
meta = rbind(meta
	,list(
	 "GSM1185585"
	,"non-smoker"
...
# Download .idat files
map2(meta$red,meta$gsm %s+% "_Red.idat.gz", ~ download.file(.x,.y) )
map2(meta$grn,meta$gsm %s+% "_Grn.idat.gz", ~ download.file(.x,.y) )
meta$red = NULL; meta$grn = NULL

# Import the methylation data
meth = read_idats(meta$gsm)
meta[,c("X","Y"):=check_sex(meth)]
meta[,sex:=ifelse(X>1.,"f","m")]

meta = meta[,.(gsm,sex,smoker)]
meta[gsm=="GSM2260573",sex:="f"]

write.csv(meta,file="meta.csv",row.names=FALSE)
```

This vignette exemplifies how to use the `ewastools` package to clean and pre-process DNA methylation data. Analysis would start with gathering the meta/phenotype data.
```{r}
meta = fread("meta.csv")
head(meta)
```

`meta` contains a column `gsm`, which in this case represents also the prefix of the .idat file names as we are using public data downloaded from the [Gene Expression Omnibus](https://www.ncbi.nlm.nih.gov/geo/) repository. In most cases, the file names will be a combination of the Sentrix barcode, Sentrix position and color channel and will look similar to this `200379120004_R01C01_Red.idat` for the .idat containing the red color channel, and like `200379120004_R01C01_Grn.idat` for the .idat containing the green color channel. `read_idats` can be used to import methylation data. It's first argument is a character vector containing the absolute or relative file path and file name but without the color channel part, e.g. `C:/folder/subfolder/200379120004_R01C01`. Both .idat files need to be in the same folder.

```{r}
meth = read_idats(meta$gsm,quiet=TRUE) # `quiet=TRUE` supresses the progress bar
```
...
chrY = meth$manifest[chr=='Y',index]
  ...
  male = which(meta$sex=="m")[1]
female = which(meta$sex=="f")[1]
...
Using the improved method gives a much more accurate result, with still all 416 Y chromosome classified as detected for the male sample, but only  `r P.neg["female"]` classified as detected for the female sample.
```{r}
P.new
```
...
Infinium BeadChips use two fluorescent dyes that are linked to the nucleotides used in the the single-base extension step. A and T nucleotides use are linked with a red dye (also called red color channel), G and C nucleotides are linked with a green dye (green color channel). Uncorrected data usually feature higher intensities in the red color channel, the so-called dye bias, resulting in beta-values above 0.5 even for semi-methylated CpG sites. Dye-bias correction normalizes the red and green color channel. `ewastools` provides an improved version of RELIC ([Xu et al., 2017](https://doi.org/10.1186/s12864-016-3426-3)) using robust Theil-Sen estimators.
...
```{r}
meta$failed = sample_failure(ctrls)
table(meta$failed)
```
...
```{r}
meta[sex!=predicted_sex,exclude:=TRUE] # flag sample
meta[sex!=predicted_sex,.(gsm,sex,predicted_sex)]
```

Another sample falls outside the two clusters.

```{r}
meta[X %between% c(0.85,0.95) & Y %between% c(0.65,0.75),.(gsm,X,Y,sex,predicted_sex)]
```
...
```{r}
meta$outlier = snp_outliers(genotypes)
meta[outlier > -4,.(gsm,X,Y,outlier)]
meta[outlier > -4,exclude:=TRUE] # flag sample
```
...
Other useful functions to be mentioned are `check_snp_agreement()` and `enumerate_sample_donors()`. The former checks if the genotypes of samples supposed to come from the same donor (or from monozygotic twins) do in fact agree; the latter returns unique IDs for unique genotypes and can, for example, be used to find technical replicates in public datasets. 

```{r}
meta$donor_id = enumerate_sample_donors(genotypes)

# List duplicates
meta[,n:=.N,by=donor_id]
meta[n>1,.(gsm,donor_id)]
meta[gsm=="GSM2260543",exclude:=TRUE] # drop duplicate
```
...
```{r fig.width=6,fig.height=6}
plot(pc2 ~ pc1,pch=ifelse(sex=="f",1,4),meta,asp=1,xlab="PC #1",ylab="PC #2")
legend("topright",pch=c(1,4),legend=c("female","male"))
```
...
```{r}
meta[pc1< -0.8,exclude:=TRUE]
meta[pc1< -0.8,.(gsm,pc1,pc2)]
```
...
```{r fig.width=6,fig.height=4}
plot(meta$GR*100,ylab="Granulocyte fraction (%)")
```

It is the second to last sample, GSM1185585.

```{r}
meta[which.max(GR),.(gsm,GR)]
meta[which.max(GR),exclude:=TRUE]
```

The lung sample is also prominent, with an estimated proportion of `GR` near zero.

```{r}
meta[which.min(GR),.(gsm,GR)]
meta[which.min(GR),exclude:=TRUE]
```

We drop the problematic samples

```{r}
drop = meta[exclude==TRUE,which=TRUE]
...
We want test all CpG sites for their association with smoking. The following code snippet is not optimized for speed but readability.

```{r}
f = function(meth){
	m = lm(meth~1+sex+smoker+GR+MO+B+CD4+CD8+NK+nRBC,data=meta)
	coef(summary(m))["smokersmoker",4]
}

f = possibly(f,otherwise=NA_real_)
...
Some of the significant CpGs are known biomarkers that have been validated in various cohorts.

## Final comments

Depending on the dataset, many other types of quality checks might be applicable. If you have suggestions or comments regarding `ewastools`, please send an [email](mailto:hhhh5@posteo.de), or file an issue or submit an pull request on GitHub (https://github.com/hhhh5/ewastools).

```{r cleanup, include=FALSE, echo=FALSE}
file.remove(meta$gsm %s+% "_Red.idat")
file.remove(meta$gsm %s+% "_Grn.idat")
