...
library(data.table)
requireNamespace("svd")
```

```{r prepare_dataset, include = FALSE, echo = FALSE, eval = TRUE}
...
# Group lines by which sample they refer to
pheno = split(pheno,cumsum(pheno %like% "^\\^SAMPLE = GSM"))

# Extract GSM accessions
names(pheno) = purrr::map(pheno,1) |> stringr::str_match(pattern = "GSM\\d+")

# Parse pheno data
pheno = pheno |>
   purrr::map(tibble::tibble) |>
   dplyr::bind_rows(.id = "gsm") |>
   tidyr::separate_wider_delim(2, delim = " = ", names = c("var", "val")) |>
   # Keep only information on sample characteristics and supplementary files
   dplyr::filter(var %in% c("!Sample_characteristics_ch1","!Sample_supplementary_file")) |>
   tidyr::separate_wider_delim(val, delim = ": ", names = c("name", "value"), too_few = "align_end") |>
   dplyr::mutate(name = dplyr::case_when(
      var == "!Sample_supplementary_file" & stringr::str_detect(value, "_Red\\.idat") ~ "red",
      var == "!Sample_supplementary_file" & stringr::str_detect(value, "_Grn\\.idat") ~ "grn",
      TRUE ~ name)) |>
   tidyr::pivot_wider(id_cols = gsm) |>
   # Select and cast the relevant variables
   dplyr::mutate(smoking = factor(`subject status`)) |>
   tibble::add_row(
      gsm = "GSM1185585",
      smoking = "non-smoker",
      red = "ftp://ftp.ncbi.nlm.nih.gov/geo/samples/GSM1185nnn/GSM1185585/suppl/GSM1185585_6285625091_R04C01_Red.idat.gz",
      grn = "ftp://ftp.ncbi.nlm.nih.gov/geo/samples/GSM1185nnn/GSM1185585/suppl/GSM1185585_6285625091_R04C01_Grn.idat.gz") |>
   tibble::add_row(
	   gsm = "GSM2219539",
      smoking = "smoker",
      red = "ftp://ftp.ncbi.nlm.nih.gov/geo/samples/GSM2219nnn/GSM2219539/suppl/GSM2219539_6222421029_R02C01_Red.idat.gz",
      grn = "ftp://ftp.ncbi.nlm.nih.gov/geo/samples/GSM2219nnn/GSM2219539/suppl/GSM2219539_6222421029_R02C01_Grn.idat.gz") |>
   dplyr::mutate(
      red = stringr::str_replace(red, "ftp:", "http:"),
      grn = stringr::str_replace(grn, "ftp:", "http:")) |>
...
# Download .idat files
purrr::map2(pheno$red, paste0(pheno$gsm, "_Red.idat.gz"), ~ download.file(.x,.y) )
purrr::map2(pheno$grn, paste0(pheno$gsm, "_Grn.idat.gz"), ~ download.file(.x,.y) )
...
Using the conventional detection p-value,
for the female samples and average of `r round(P.neg[["f"]])` Y chromosome probes are called detected
and a number close to all 416 Y chromosome probes as for the male sample.

```{r}
P.neg
```

Using the improved method gives a much more accurate result
with an average of `r round(P.new[["m"]])` Y chromosome probes classified as detected for the male sample,
but only  `r round(P.new[["f"]])` probes classified as detected for the female sample.
More information can be found in [Heiss and Just, 2019](https://doi.org/10.1186/s13148-019-0615-3).

```{r}
P.new
```

We used a significance threshold of 0.01 above.
Moving forward, probes above this threshold should be masked, i.e., set to missing.
...
Depending on the dataset, many other types of quality checks might be applicable. If you have suggestions or comments regarding `ewastools`, please send an [email](mailto:heiss.jonathan@gmail.com), or file an issue or submit a pull request on GitHub (https://github.com/hhhh5/ewastools).

```{r cleanup, include=FALSE, echo=FALSE, eval = TRUE}
