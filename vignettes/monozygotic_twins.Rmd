...
## Filtering using detection p-values

`ewastools` implements an improved estimation of background signal (implemented in the function `detectionP`) that leads to much fewer Y chromosome probes being called among females compared to the conventional way using negative control probes (implemented in `detectionP.neg`). The same cut-off of 0.01 was used both times.

```{r fig.width=6,fig.height=4}
chrY = meth$manifest[chr=='Y',index]
females = samples$predicted_sex == "f"

## conventional
P.neg = meth %>% detectionP.neg
P.neg = P.neg[chrY,females]
P.neg = colSums(P.neg<0.01,na.rm=TRUE)

## non-specific fluorescence
meth %<>% detectionP

P.nsp = meth$detP[chrY,females]
P.nsp = colSums(P.nsp<0.01,na.rm=TRUE)

boxplot(list(P.neg,P.nsp),names=c("Negative\ncontrols","Non-specific\nfluorescence"),horizontal=TRUE,xlab="Number of detected Y-chromosome probes among females")
```

Mask undetected probes and compute beta-values. No normalization is performed.
```{r}
meth %<>% mask(0.01)
